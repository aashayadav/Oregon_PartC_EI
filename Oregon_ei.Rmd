---
title: "Part C EI Eligibility for Diagnosed Medical Conditions in Oregon"
output: 
...
data <- data %>%
  mutate(position = recode(position,
                           "Assistant teacher" = "EI/ECSE Personnel",
                           "EI coordinator" = "EI/ECSE Personnel",
                           "EI/ECSE Coordinator" = "EI/ECSE Personnel",
                           "EI/ECSE director" = "EI/ECSE Personnel",
                           "EI/ECSE specialist" = "EI/ECSE Personnel",
                           "EI/ECSE specialist autism" = "EI/ECSE Personnel",
                           "EI/ECSE specialist, & ECSE ABA Teacher" = "EI/ECSE Personnel",
                           "EI/ECSE specialist, EI coordinator" = "EI/ECSE Personnel",
                           "EI/ECSE specialist, EI/ECSE specialist autism" = "EI/ECSE Personnel",
                           "Eval Team Manager" = "EI/ECSE Personnel",
                           "Intake Specialist who works w/med statement referrals" = "EI/ECSE Personnel",
                           "Nurse practitioner" = "Healthcare Practitioner",
                           "Physical therapist, Evaluator-- evaluation team" = "EI/ECSE Personnel",
                           "Physician" = "Healthcare Practitioner",
                           "Physician assistant" = "Healthcare Practitioner",
                           "Program coordinator" = "EI/ECSE Personnel",
                           "Senior advisor" = "EI/ECSE Personnel",
                           "State EI/ECSE staff" = "EI/ECSE Personnel",
                           "EI professional" = "EI/ECSE Personnel",
                           "Healthcare professional" = "Healthcare Practitioner",
                           "Physical therapist" = "EI/ECSE Personnel",
                           "Occupational therapist" = "EI/ECSE Personnel",
                           "Speech & Language pathologist" = "EI/ECSE Personnel"))
...
table_data %>%
  ...
  pack_rows("Profession", 1, 2, label_row_css = "background-color: #F0FFFF; color: black;") %>%
  pack_rows("Job Status", 3, 4, label_row_css = "background-color: #F0FFFF; color: black;") %>%
  pack_rows("Role", 5, 6, label_row_css = "background-color: #F0FFFF; color: black;") %>%
  pack_rows("Years of Service", 7, 8, label_row_css = "background-color:#F0FFFF; color: black;") %>%
...
aware_fisher <- data.frame(
  ...
  "EI/ECSE Personnel Vs Healthcare Practitioner" = c("0.0001***", "0.001***", "0.001***", "2.295"),
...
aware_fisher %>%
  kableExtra::kable(col.names = c("Ques", "EI/ECSE Personnel\n Vs\nHealthcare Practitioner",
                                  "Full-time\nVs\nPart-time",
...
ggplot(aware_plot,
  ...
  geom_text(aes(x = percent/2, y = aware, label = paste0(abs(aware_plot$percent), "%")), color = "red", 
            size = 4) +
  theme_xaringan() +
  ggtitle("Awareness: All Participants") +
...
plot_aware_byprofession <- ggplot(data_pos,
       aes(x = percent, y = fct_reorder(aware, percent), fill = response)) +
  geom_bar(aes(fill = response), stat = "identity", position = "identity", width = 0.6) +
  facet_wrap(~position) +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed") +
  colorblindr::scale_fill_OkabeIto(name = "response") +
  ...
  ggtitle("(a) Awareness/knowledge: By Profession") +
  ...
  theme(axis.title = element_blank(),
        ...
        plot.title.position = "plot",
        axis.text=element_text(size=12, color = "black"),
        plot.title = element_text(color= "red", #"red", 
                                  size=12, 
                                  hjust = 0.5),
        legend.text = element_text(size = 12),
        strip.background = element_blank(),
        strip.text = element_text(size = 12, face = "bold", #face="bold",
                                  color = "#d55e00", #"#d55e00", 
                                  hjust = 0.5)) +  #+
  guides(fill = guide_legend(reverse = TRUE)) 
 # scale_fill_manual(values = c("#B8B8B8", "#D0D0D0"))
  

plot_aware_byprofession

```
```{r}
# save the plot in same directory.
png(file="plot_aware_byprofession.png")
plot_aware_byprofession
dev.off()
```
...
prac_1_all <- data %>%
  ...
  count() %>%
  rename("Skipped EI Eligibility Eval" = n,
...
prac_5_all <- data %>%
  ...
  count() %>%
  rename("Searched for eligible medical \n conditions" = n,
...
In_practice_all<-ggplot(practice_data,
       aes(x = percent, y = fct_reorder(practice, percent), fill = response)) +
  geom_bar(aes(fill = response), stat = "identity", position = "identity", width = 0.6) +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed") + #"red", linetype = "dashed") +
  colorblindr::scale_fill_OkabeIto(name = "response") +
  scale_x_continuous(breaks = pretty(practice_data$percent), 
                     labels = paste0(abs(pretty(practice_data$percent)), "%")) +
  geom_text(aes(x = percent/2, y = practice, label = paste0(abs(practice_data$percent), "%")), 
            color = "black", #"red", 
            size = 4) +
 # theme_xaringan() +
  theme_minimal() +
  ggtitle("(b) In Practice: All Participants") +
 # labs(caption = "Figure 2") +
   theme(axis.title = element_blank(),
         ...
         plot.title.position = "plot",
         axis.text=element_text(size=12, color = "black"),
         plot.title = element_text(color= "black",size=12, hjust = 0.5), # "red", size=12, hjust = 0.5),
         legend.text = element_text(size = 12)) +
  guides(fill = guide_legend(reverse = FALSE)) 
  #scale_fill_manual(values = c("#D0D0D0", "#B8B8B8"))
...
In_practice_by_profession <- ggplot(practice_data_pro,
       aes(x = percent, y = fct_reorder(practice, percent), fill = response)) +
  geom_bar(aes(fill = response), stat = "identity", position = "identity", width = 0.4) +
  facet_wrap(~position) +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed") + 
  ...
  geom_text(aes(x = percent/2, y = practice, label = paste0(abs(practice_data_pro$percent), "%")), color = "black", #"red", 
            size = 3) +
  theme_xaringan() +
  ggtitle("In Practice (By Profession)") +
   theme(axis.title = element_blank(),
         panel.grid.minor = element_blank(),
         legend.title = element_blank(),
         legend.position = "bottom",
         ...
         plot.title.position = "plot",
         axis.text=element_text(size=12, color = "black"),
         plot.title = element_text(color= "black", size=15, hjust = 0.5), #"red"
         legend.text = element_text(size = 15),
         strip.background = element_blank(),
         strip.text = element_text(face="bold", size = 15, color = "#d55e00", hjust = 0.5)) + #d55e00", hjust = 0.5)) +
  guides(fill = guide_legend(reverse = FALSE))
 


In_practice_by_profession

```

```{r}
# save the plot in same directory.
pdf(file="Inpractice_byprofession.pdf")
In_practice_by_profession
dev.off()
```
...
ggplot(practice_data_job,
  ...
  ggtitle("Use of Listed Medical Conditions By Job Status") +
   theme(axis.title = element_blank(),
         ...
         legend.text = element_text(size = 15),
         strip.background = element_blank(),
         strip.text = element_text(face="bold", size = 15, color = "#d55e00", 
                                   hjust = 0.5)) +
  guides(fill = guide_legend(reverse = FALSE))
...
covid_allparticipants <- ggplot(cov_med_ei,
  ...
  #theme_xaringan_inverse() +
  theme_xaringan() +
  ggtitle("During COVID-19 Pandemic (All Participants)") +
...
covid_allparticipants

```

```{r}
# save the plot in same directory.
pdf(file="covid_allparticipants.pdf")
covid_allparticipants
dev.off()
```
...
data_onepro <- data_covid %>%
  ...
  filter(position == "EI/ECSE Personnel") %>%
  drop_na()
...
covid_only_ei <- covid_only_ei %>%
  pivot_longer(cols = c("Using parent-administered\n tool only",
               ...
               names_to = "during_pandemic",
               values_to = "count") %>%
  mutate(percent = round((count/37)*100, digits = 0))
...
covid_eiecse <- ggplot(covid_only_ei,
       aes(x = percent, y = fct_reorder(during_pandemic, percent), fill = response)) +
  geom_bar(aes(fill = response), stat = "identity", position = "identity", width = 0.6) +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed") +
  colorblindr::scale_fill_OkabeIto(name = "response") +
  ...
  geom_text(aes(x = percent/2, y = during_pandemic, label = paste0(abs(covid_only_ei$percent),
                                                                   "%")), color = "black", 
            size = 4) +
...
# theme_xaringan_inverse() +
  #theme_xaringan() + 
  theme_minimal() +
  ggtitle("(c) Eligibility Evaluation During COVID-19: Part C EI/ECSE Personnel") +
...
recommendation <- ggplot(data_reccomended,
                         aes(x = percent, y = fct_reorder(information, percent), fill = response)) +
  geom_bar(aes(fill = response), stat = "identity", position = "identity", width = 0.6) +
  facet_wrap(~position) +
  geom_vline(xintercept = 0, color = "#A0A0A0", linetype = "dashed") +
  colorblindr::scale_fill_OkabeIto(name = "response") +
  scale_x_continuous(breaks = pretty(data_reccomended$percent),
                     labels = paste0(abs(pretty(data_reccomended$percent)), "%")) +
  geom_text(aes(x = percent/2, y = information, 
                label = paste0(abs(data_reccomended$percent), "%")), color = "black", 
            size = 4) +
  #theme_xaringan() +
  theme_minimal() +

 # theme_xaringan_inverse() +
  ggtitle("(d) Recommendations/Views: All Participants") +
   theme(axis.title = element_blank(),
         panel.grid.minor = element_blank(),
         legend.title = element_blank(),
         legend.position = "bottom",
         ...
         strip.background = element_blank(),
         strip.text = element_text(size = 13, color = "black", hjust = 0.5)) +
  scale_fill_manual(values = c("#D0D0D0", "#B8B8B8"))
...
article_plot <- plot_aware_byprofession + In_practice_all + 
  covid_eiecse + recommendation 
article_plot
